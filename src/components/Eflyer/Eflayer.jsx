...
const Eflayer = () => {
	...
	const handleDelete = async (id) => {
		try {
			...
			setEflyers(eflyers.filter((eflyer) => eflyer._id !== id)); // Update the state after deletion
			setFilteredEflyers(filteredEflyers.filter((eflyer) => eflyer._id !== id)); // Update filtered list
		} catch (error) {
							...
							<tbody className='divide-y divide-gray-700'>
								{filteredEflyers.map((eflyer, index) => (
									...
									>
										...
										<td className='px-6 py-4 whitespace-nowrap'>{eflyer.category.

Category_Name
}</td>
										<td className='px-6 py-4 whitespace-nowrap'>{eflyer.
										...
										<td className='px-6 py-4 whitespace-nowrap text-sm text-gray-300'>
											<button className='text-indigo-400 hover:text-indigo-300 mr-2' onClick={() => handleUpdate(eflyer._id)}>Edit</button>
											<button className='text-red-400 hover:text-red-300' onClick={() => handleDelete(eflyer._id)}>
												Delete
											</button>
										</td>
									</motion.tr>
